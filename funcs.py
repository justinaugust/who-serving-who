...
#     replace_star = sal.sql.text("""
#     UPDATE "public"."caaspp_2019" 
#     SET "Total Tested with Scores"='0' 
#     WHERE ctid IN 
#     (SELECT ctid 
#         FROM "public"."caaspp_2019" 
#         WHERE "Total Tested with Scores"='*') 
#         RETURNING "County Code", "District Code", "School Code", 
#                 "Filler", "Test Year", "Subgroup ID", "Test Type", 
#                 "Total Tested At Entity Level", "Total Tested with Scores", 
#                 "Grade", "Test Id", "CAASPP Reported Enrollment", 
...
#     replace_star = sal.sql.text("""
#     UPDATE "public"."caaspp_2019" 
#     SET "Percentage Standard Met and Above"='0' 
#     WHERE ctid IN 
#     (SELECT ctid 
#         FROM "public"."caaspp_2019" 
#         WHERE "Percentage Standard Met and Above"='*') 
#         RETURNING "County Code", "District Code", "School Code", 
#                 "Filler", "Test Year", "Subgroup ID", "Test Type", 
#                 "Total Tested At Entity Level", "Total Tested with Scores", 
#                 "Grade", "Test Id", "CAASPP Reported Enrollment", 
...
#             INNER JOIN caaspp_tests tests
#                 ON res2019."Test Id" = tests."Test ID"
#             INNER JOIN caaspp_subgroups subgroups
#                 ON subgroups."Demographic ID" = res2019."Subgroup ID"
...
with engine.connect() as conn, conn.begin():
    ...
    SELECT
        ...
        WHERE
            CAST(meta."Low Grade*" AS FLOAT) > -1
...
with engine.connect() as conn, conn.begin():
    ...
    SELECT
        ...
        WHERE
            CAST(meta."Low Grade*" AS FLOAT) > -1
        GROUP BY school_level, test_id, demographic_group, subgroup_id, demographic
    ;
    """)
    city_scores = pd.read_sql(command, con=conn)
    conn.execute("commit")
...
for school_level in disagg_scores.school_level.unique():
    for subgroup in disagg_scores.subgroup_id.unique():
        for test_id in disagg_scores.test_id.unique():
            mask = (disagg_scores['subgroup_id'] == subgroup) &             (disagg_scores['school_level'] == school_level) &             (disagg_scores['test_id'] == test_id)
...
df1 = disagg_scores.groupby(by=['school_level', 'test_id', 'subgroup_id'])[['total_tested']].sum()
df2 = disagg_scores.groupby(by=['school_level','test_id', 'subgroup_id'])[['prof_above']].mean()
total_tested = pd.concat([df1,df2], axis=1, )
           
total_tested.reset_index(inplace=True)
for school_level in total_tested.school_level.unique():
    for subgroup in total_tested.subgroup_id.unique():
        for test_id in total_tested.test_id.unique():
            
            all_students = total_tested.loc[(total_tested['test_id'] == test_id) &              (total_tested['school_level'] == school_level) &             (total_tested['subgroup_id'] == 1), 'prof_above'].values[0]

            sub_prof = total_tested.loc[(total_tested['test_id'] == test_id) &                          (total_tested['school_level'] == school_level) &                         (total_tested['subgroup_id'] == subgroup), 'prof_above'].values[0]
            total_tested.loc[(total_tested['test_id'] == test_id) &                          (total_tested['school_level'] == school_level) &                         (total_tested['subgroup_id'] == subgroup), 'demo_diff'] = sub_prof - all_students
total_tested.head(3)
...
big_total_tested = pd.merge(left = total_tested,
                            right = subgroups_df,
                            left_on = 'subgroup_id',
                           right_on = 'Demographic ID')
big_total_tested = big_total_tested.merge(right = tests_df,
                      left_on = 'test_id',
                      right_on = 'Test ID')
...
tests = list(disagg_scores['test'].unique())
...
def school_level_map(school_level):
    schools = disagg_scores.copy()
    schools = schools[schools['school_level'] == school_level]
...
def display(identity = {'all_students':1}, school_level = ['elementary','middle','high']):
    ...
    test1_all_stud_prof = total_tested.loc[(total_tested['school_level'].isin(school_level)) & (total_tested['subgroup_id'] == 1) & (total_tested['test_id'] == 1), 'prof_above'].values[0]
    test2_all_stud_prof = total_tested.loc[(total_tested['school_level'].isin(school_level)) & (total_tested['subgroup_id'] == 1) & (total_tested['test_id'] == 2), 'prof_above'].values[0]
